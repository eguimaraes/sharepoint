...
export default class MostraPerfilSharePointWebPart extends BaseClientSideWebPart<IMostraPerfilSharePointWebPartProps> {
  ...
  private _getListData(): Promise<IDado> {
    ...
    return this.context.spHttpClient.get(this.context.pageContext.web.absoluteUrl + `/_api/search/query?querytext='*'&SelectProperties='AboutMe,AccountName,UserProfile_GUID,LastModifiedTime,OriginalPath,Path,PreferredName,ServiceApplicationID,WorkEmail,PictureURL,SipAddress,UserLocation'&TrimDuplicates=true&SourceId='B09A7990-05EA-4AF9-81EF-EDFAB16C4E31'&rowsperpage=100&RowLimit=300'`, clientConfigODataV3)
  ...
  private _renderList(item: IDado): void {  
    ...
    linhas.map(linha => {           
      ...
      linha.Cells.map(dadosV => {
        ...
        let campos:any = ["PreferredName","AboutMe","PictureURL"];
        ...
        if (campos.includes(dadosV.Key) && dadosV.Value !== null) {
          ...
          switch (dadosV.Key){
            ...
            default:
              html += `<div> ${dadosV.Value}</div>`; 
            break
              
            case "PictureURL":
              html += `<div><img src='${dadosV.Value}'></div>`;
            break
